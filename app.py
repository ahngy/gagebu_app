...
def find_row_by_field(sheet_key: str, field: str, value: str):
    h = headers(sheet_key)
    if field not in h:
        return None
    idx = h.index(field) + 1
    ...
    w = ws(sheet_key)
    vals = with_retry(lambda: w.get(f"{a1}2:{a1}"))
    col = [r[0] for r in vals if r]
    try:
        pos0 = col.index(value)
        return 2 + pos0
    except ValueError:
        return None

def update_row_by_rownum(sheet_key: str, row_num: int, updates: dict):
    w = ws(sheet_key)
    h = headers(sheet_key)
    cell_objs = []
    for k, v in updates.items():
        if k not in h:
            continue
        c = h.index(k) + 1
        cell_objs.append(gspread.Cell(row_num, c, v))
    if not cell_objs:
        return False, "업데이트할 컬럼이 없습니다(헤더 확인)."
    with_retry(lambda: w.update_cells(cell_objs, value_input_option="USER_ENTERED"))
    return True, "수정 완료"

def delete_row_by_rownum(sheet_key: str, row_num: int):
    w = ws(sheet_key)
    with_retry(lambda: w.delete_rows(row_num))
    return True, "삭제 완료"

def _make_editor(df, cols_show, cols_edit, key):
    """Create a data_editor frame with a delete checkbox."""
    d = df.copy()
    d["삭제"] = False
    show_cols = cols_show + ["삭제"]
    return st.data_editor(
        d[show_cols],
        hide_index=True,
        use_container_width=True,
        disabled=[c for c in show_cols if c not in cols_edit and c != "삭제"],
        key=key,
    )
...
with tabs[1]:
    ...
    if b.empty:
        ...
        st.markdown("#### 표에서 바로 수정/삭제")
        b_inline = b.copy()
        b_inline["target"] = pd.to_numeric(b_inline["target"], errors="coerce").fillna(0).astype(int)
        edited = _make_editor(
            b_inline.rename(columns={"category":"카테고리","target":"목표금액"}).assign(카테고리=b_inline["category"].astype(str)),
            cols_show=["카테고리","목표금액"],
            cols_edit=["목표금액"],
            key="budget_inline_editor",
        )
        if st.button("✅ 변경사항 저장/삭제 반영", type="primary", key="budget_inline_apply"):
            orig = b_inline.set_index("id")
            for i, row in edited.iterrows():
                rid = str(b_inline.iloc[i]["id"])
                if bool(row.get("삭제", False)):
                    delete_row_by_id("budgets", rid)
                    continue
                t = to_int_amount(row.get("목표금액", ""))
                if t is None:
                    try:
                        t = int(row.get("목표금액"))
                    except:
                        st.error(f"목표금액 오류: {row.get('목표금액')}")
                        st.stop()
                cat = str(b_inline.iloc[i]["category"])
                dk = f"BUDGET|{bym}|{cat}|{int(t)}"
                if int(orig.loc[rid].get("target",0)) != int(t):
                    update_row_by_id("budgets", rid, {"target": int(t), "dedup_key": dk})
            read_df.clear()
            st.success("반영 완료")
...
with tabs[2]:
    ...
    if a.empty:
        ...
        st.markdown("#### 표에서 바로 수정/삭제 (반영내역 + 가계부 연동)")
        a_inline = a.copy()
...
with tabs[5]:
    ...
    if cards.empty:
        ...
        c_inline = cards.copy()
        edited = _make_editor(
            c_inline.rename(columns={"card_name":"카드명","benefit_memo":"혜택 메모"}),
            cols_show=["카드명","혜택 메모"],
            cols_edit=["카드명","혜택 메모"],
            key="cards_inline_editor",
        )
        if st.button("✅ 변경사항 저장/삭제 반영", type="primary", key="cards_inline_apply"):
            orig = c_inline.set_index("id")
            for i, row in edited.iterrows():
                rid = str(c_inline.iloc[i]["id"])
                if bool(row.get("삭제", False)):
                    delete_row_by_id("cards", rid)
                    continue
                name = str(row.get("카드명","")).strip()
                memo = str(row.get("혜택 메모","")).strip()
                if not name:
                    st.error("카드명은 비울 수 없습니다.")
                    st.stop()
                o = orig.loc[rid]
                if str(o.get("card_name","")) != name or str(o.get("benefit_memo","")) != memo:
                    update_row_by_id("cards", rid, {"card_name": name, "benefit_memo": memo})
            read_df.clear()
            st.success("반영 완료")
